...
export class AbstractClient {
  ...
  constructor(
    ...
    if (providerName) {
      ...
      if (!provider) {
        throw new Error(
          `Provider '${providerName}' not found. Available providers: ${config
            .getAllProviders()
  ...
  protected async makePostRequest<T>(
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (
          errorData?.cause === 'A security-sensitive error has been received'
        ) {
          ...
          try {
            const retryResponse = await this.axiosInstance.post<T>(url, data, {
              headers: newHeaders,
            });
            ...
            await this.logRequestResponse(url, data, {
              error: axios.isAxiosError(retryError)
