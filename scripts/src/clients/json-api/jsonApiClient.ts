...
export class JsonApiClient extends AbstractClient {
  ...
  async createCommand(
    ...
    try {
      ...
      if (!('CreatedTreeEvent' in event)) {
        throw new Error(
          `Expected CreatedTreeEvent but got ${Object.keys(event)[0]}`
        );
      }
  ...
  async exerciseCommand(
    params: ExerciseCommandParams
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (errorData?.cause?.includes('Party already exists')) {
          ...
          const existingParty = parties.partyDetails.find(p =>
            p.party.startsWith(`FM:${partyIdHint}`)
          );
          if (existingParty) {
            // Set user rights for the newly created party
  ...
  private async getParties(): Promise<{
    partyDetails: Array<{
      party: string;
      isLocal: boolean;
      localMetadata: {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        const errorData = error.response?.data
          ...
          : error.message;
        throw new Error(
          `Failed to get transaction tree by offset: ${errorData}`
        );
      }
      throw error;
    }
    ...
    try {
      ...
      if (options?.includeCreatedEventBlob !== undefined) {
        queryParams.append(
          ...
          options.includeCreatedEventBlob.toString()
        );
      }
