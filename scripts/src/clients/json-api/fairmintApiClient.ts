...
const TEMPLATES = {
  FAIRMINT_ADMIN_SERVICE:
    '#OpenCapTable-v01:FairmintAdminService:FairmintAdminService',
} as const;
...
export class FairmintClient {
  ...
  async createFairmintAdminService(): Promise<CreateContractResponse> {
    ...
    console.debug(
      `Created FairmintAdminService with contract ID: ${response.contractId}`
    );
    return response;
  }
  ...
  async authorizeIssuer(
    ...
    this.ledger = new Ledger({
      ...
      httpBaseUrl: this.client.provider.JSON_API.API_URL.substring(
        0,
        this.client.provider.JSON_API.API_URL.length - 2
      ),
    ...
    const authorizationContractId =
      response.transactionTree.eventsById['0'].ExercisedTreeEvent.value
        .exerciseResult;
    console.debug(
      `Successfully authorized issuer with contract ID: ${authorizationContractId}`
    );
  ...
  async createParty(partyIdHint: string) {
    ...
    console.debug(
      `${response.isNewParty ? 'Created' : 'Reused'} party for ${partyIdHint} with ID: ${response.partyId}`
    );
    return response;
  }
  ...
  async acceptIssuerAuthorization(
    ...
    const issuerContractId =
      response.transactionTree.eventsById['1'].CreatedTreeEvent.value
        .contractId;
    console.debug(
      `Successfully created issuer with contract ID: ${issuerContractId}`
    );
    return issuerContractId;
  }

  async createStockClass(
    issuerContractId: string,
    stockClassType: string,
    shares: number,
    issuerPartyId: string
  ): Promise<{
    stockClassContractId: string;
    updatedIssuerContractId: string;
  }> {
    const response = (await this.client.exerciseCommand({
    ...
    const stockClassContractId =
      response.transactionTree.eventsById['2'].CreatedTreeEvent.value
        .contractId;
    const updatedIssuerContractId =
      response.transactionTree.eventsById['1'].CreatedTreeEvent.value
        .contractId;
    console.debug(
      `Created stock class with contract ID: ${stockClassContractId}`
    );
    return { stockClassContractId, updatedIssuerContractId };
  }

  async proposeIssueStock(
    stockClassContractId: string,
    ...
    const proposalContractId =
      response.transactionTree.eventsById['1'].CreatedTreeEvent.value
        .contractId;
    const updatedStockClassContractId =
      response.transactionTree.eventsById['2'].CreatedTreeEvent.value
        .contractId;
    console.debug(
      `Proposed stock issuance to ${recipientPartyId} with proposal ID: ${proposalContractId}`
    );
    return { proposalContractId, updatedStockClassContractId };
  }

  async acceptIssueStockProposal(
    proposalContractId: string,
    recipientPartyId: string
  ): Promise<string> {
    const response = (await this.client.exerciseCommand({
    ...
    const stockPositionContractId =
      response.transactionTree.eventsById['1'].CreatedTreeEvent.value
        .contractId;
    console.debug(
      `${recipientPartyId} accepted stock issuance and received position with ID: ${stockPositionContractId}`
    );
  ...
  async proposeTransfer(
    ...
    transferProposalContractId: string;
    ...
    const transferProposalContractId =
      response.transactionTree.eventsById['1'].CreatedTreeEvent.value
        .contractId;
    const updatedStockPositionContractId =
      response.transactionTree.eventsById['2'].CreatedTreeEvent.value
        .contractId;
    console.debug(
      `${ownerPartyId} proposed transfer to ${recipientPartyId} with proposal ID: ${transferProposalContractId}`
    );
  ...
  async acceptTransfer(
    ...
    const stockPositionContractId =
      response.transactionTree.eventsById['1'].CreatedTreeEvent.value
        .contractId;
    console.debug(
      `${recipientPartyId} accepted transfer and received position with ID: ${stockPositionContractId}`
    );
    return stockPositionContractId;
  }
