...
async function main() {
  try {
    ...
    const authorizationContractId = await client.authorizeIssuer(
      contractId,
      issuerPartyId
    );
    ...
    const bobStockPositionContractId = await client.acceptIssueStockProposal(
      proposalContractId,
      bobPartyId
    );
    ...
    const aliceStockPositionContractId = await client.acceptTransfer(
