...
async function buildPackage(
  ...
  try {
    ...
    throw new Error(
      `Failed to build package ${packageName}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}

async function findDarFile(
  packagePath: string,
  packageName: string
): Promise<string> {
  ...
  if (!fs.existsSync(darPath)) {
    ...
    if (darFiles.length > 1) {
      console.warn(
        `Multiple .dar files found in ${damlDistPath}, using last one: ${darFiles[darFiles.length - 1]}`
      );
    }
...
async function uploadDarFile(
  ...
  try {
    ...
    throw new Error(
      `Failed to upload DAR file ${darPath}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function uploadPackage(options: UploadOptions): Promise<void> {
  ...
  try {
    ...
    console.error(
      `❌ Failed to upload package ${packageName}:`,
